        ...
        public string[] GetTemplatesWithParams()
        {
            ...
            for (int i = 0; i < templTitles.Length; i++)
                while (i < templates.Count &&
                    !templates[i].Trim().StartsWith(templTitles[i]) &&
                    !templates[i].Trim().StartsWith(site.namespaces["10"].ToString() + ":" +
                        templTitles[i], true, site.langCulture) &&
                    !templates[i].Trim().StartsWith(Site.wikiNSpaces["10"].ToString() + ":" +
                        templTitles[i], true, site.langCulture) &&
                    !templates[i].Trim().StartsWith("msgnw:" + templTitles[i]))
